...
## Share Landing (`/m/{id}`, ROA-192)

Public OG-previewable meetup landing, outside the `[locale]` segment (viewer locale from `Accept-Language`, fallback en). Locked mockups: `~/.gstack/projects/terrykang90-roami/designs/share-landing-20260609/`.

**Card pattern** (`src/app/m/[id]/`): CSS mini-map header (warm `--color-map-*` palette, decorative blobs `aria-hidden`, teal pin with category emoji) + category pill (category color) + title (2-line clamp) + description (3-line clamp mobile / 4 desktop, omitted entirely when empty) + date row + host trust block. Host trust = tier chip (`share.tier_*`) + hosted count + 👍 recommends — counts hidden at 0, NO numeric star rating exists. Avatar fallback = monogram on teal-light; missing nationality = no flag.
...
**CTA matrix** (`resolveCta`): 플랫폼별 launch state (`PlatformLaunch {ios, android}`, env `NEXT_PUBLIC_IOS_LAUNCH_STATE`/`NEXT_PUBLIC_ANDROID_LAUNCH_STATE` — plan 005) × UA platform. prelaunch: iOS→TestFlight (`NEXT_PUBLIC_TESTFLIGHT_URL`), Android→`/{locale}/android` self-onboarding link (Google Group → opt-in → Play 설치); iOS만 launched (혼합): iOS→App Store 단일 배지(`app_store` variant), Android는 베타 유지; 양쪽 launched: store badges (`NEXT_PUBLIC_APP_STORE_URL`/`NEXT_PUBLIC_PLAY_STORE_URL`). Desktop (≥768px) = 2-col, card max 480px + sticky get-app panel with server-generated QR (`qrcode` lib — never an external QR API).
