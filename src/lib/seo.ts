...
export function storeInstallUrls(launch: PlatformLaunch, appStore: string, playStore: string) {
  const urls: string[] = []
  if (launch.ios === 'launched') urls.push(appStore)
  if (launch.android === 'launched') urls.push(playStore)
