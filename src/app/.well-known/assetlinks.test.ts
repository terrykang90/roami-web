...
//
// Lists both the Play App Signing key SHA-256 (Play-distributed installs are
// re-signed with Google's key) and the upload key SHA-256 (local/upload-signed
// builds). Both must be valid colon-hex — a malformed entry poisons the whole
// statement list and silently breaks App Link auto-verification even for the
// valid key. AASA has applinks.test.ts; this is the assetlinks equivalent.
